...
self.onmessage = (e: MessageEvent) => {
  const { type, data } = e.data as {
    type: string;
    data: { pixels: Uint8ClampedArray; pixW: number; pixH: number; opts: AsciiOptions };
  };

  if (type === "reset") { prevSmoothed = null; return; }
  if (type !== "frame") return;

  const { pixels: px, pixW, pixH, opts } = data;
  const { asciiW, asciiH, brightness, contrast, threshold, gamma,
  ...
  if (brailleMode) {
    ...
    const htmlLines: string[] = [], indices: number[][] = [];
    for (let cy = 0; cy < bH; cy++) {
      ...
      for (let cx = 0; cx < bW; cx++) {
        ...
        const ch = String.fromCodePoint(BRAILLE_BASE|bits); row.push(bits);
  ...
  if (blockMode) {
    ...
    for (let y = 0; y < asciiH; y++) {
      ...
      for (let x = 0; x < asciiW; x++) {
        ...
        const ch = blocks[idx]; row.push(idx);
  ...
  const { mag, dir } = needGrad ? sobelGradient(proc, pixW, pixH) : { mag: proc, dir: new Float32Array(N) };
  ...
  for (let y = 0; y < asciiH; y++) {
    ...
    for (let x = 0; x < asciiW; x++) {
      ...
      row.push(charIdx);
      const out = charOut!=="\u00a0" && charOut===" " ? "\u00a0" : charOut!;
